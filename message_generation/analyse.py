...
class Analyse:
    ...
    def find_duplicate_messages(self, file_path):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if 'bgp' in packet['layers'].keys():
                    ...
                    # if not UPDATE message, skip
                    if item_bgp.get('bgp_bgp_type') == "2":
                        message_found = 0
                        # go through all messages
                        for message in self.relevant_messages:
                            if message.get('bgp') == item_bgp:
                                #print("Message found in relevant messages.")
                                message_found = 1
        ...
        with open(f'{results_directory}timestamp_list_{file_name}.csv', 'w') as output:
